...
export function SignupForm() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Une erreur est survenue avec Google';
      setError(errorMessage)
      setLoading(false)
    }
  }
  ...
  const handleAppleSignup = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Une erreur est survenue avec Apple';
      setError(errorMessage)
      setLoading(false)
    }
  }
    ...
    <div className="w-full max-w-sm sm:max-w-md lg:max-w-md xl:max-w-md mx-auto">
      ...
      <div className="bg-card/60 backdrop-blur-xl border border-border/50 rounded-2xl sm:rounded-3xl lg:rounded-[2rem] p-6 sm:p-8 lg:p-10 xl:p-12 shadow-2xl shadow-primary/5">
        <div className="text-center mb-6 lg:mb-8">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-2 lg:mb-4">Créer un compte</h2>
          <p className="text-sm sm:text-base lg:text-lg text-muted-foreground">
            Rejoignez des milliers d&apos;entreprises qui optimisent leur inventaire
          </p>
        </div>
        ...
        {/* Signup Form */}
        <form onSubmit={handleEmailSignup} className="space-y-4 sm:space-y-6 lg:space-y-8">
          {/* Name Fields */}
          <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:gap-6">
            <div className="space-y-2 lg:space-y-3">
              <label htmlFor="firstName" className="text-sm lg:text-base font-medium text-foreground">
                Prénom
              </label>
              <div className="relative">
                <User className="absolute left-3 sm:left-4 lg:left-5 top-1/2 transform -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-muted-foreground" />
                <input
                  id="firstName"
                  type="text"
                  placeholder="Jean"
            ...
            <div className="space-y-2 lg:space-y-3">
              <label htmlFor="lastName" className="text-sm lg:text-base font-medium text-foreground">
                Nom
              </label>
              <div className="relative">
                <User className="absolute left-3 sm:left-4 lg:left-5 top-1/2 transform -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-muted-foreground" />
          ...
          <div className="space-y-2 lg:space-y-3">
            <label htmlFor="confirmPassword" className="text-sm lg:text-base font-medium text-foreground">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 sm:right-4 lg:right-5 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors"
              >
                {showConfirmPassword ? <EyeOff className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" /> : <Eye className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" />}
              </button>
            </div>
          </div>
